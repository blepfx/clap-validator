...
pub const PARAM_DRIVE_ID: ClapId = ClapId::new(4);
...
impl GainParams {
    pub fn new() -> Self {
        Self {
            ...
            drive: AtomicF32::new(DEFAULT_DRIVE),
        }
    }
    ...
    pub fn get_drive(&self) -> f32 {
        self.drive.load(Ordering::Relaxed)
    }
    ...
    #[inline]
    pub fn set_drive(&self, new_drive: f32) {
        let new_drive = new_drive.clamp(0., 2.);
        self.drive.store(new_drive, Ordering::Relaxed)
    }
    ...
    pub fn handle_event(&self, event: &UnknownEvent) {
        if let Some(CoreEventSpace::ParamValue(event)) = event.as_core_event() {
            ...
            } else if event.param_id() == PARAM_DRIVE_ID {
                self.set_drive(event.value() as f32);
            }
        }
    }
}
...
impl PluginStateImpl for GainPluginMainThread<'_> {
    fn save(&mut self, output: &mut OutputStream) -> Result<(), PluginError> {
        output.write_all(&self.shared.params.get_volume().to_le_bytes())?;
        output.write_all(&self.shared.params.get_drive().to_le_bytes())?;
